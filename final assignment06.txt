...
# Kstevens,11-13-19,Modified code to complete assignment 6
# ------------------------------------------------------------------------ #
...
class FileProcessor:
    ...
    @staticmethod
    def RemoveListDataFromFile(file_name, list_of_rows):
        """
        Desc - Removes data from a file of a list of dictionary rows
...
FileProcessor.ReadFileDataToFile(strFileName, lstTable)  # read file data
...
while(True):
    ...
    # Step 3.3 - Remove a new item to the list/Table
        ...
        # ToDo/done: Place processing code in a new function
        FileProcessor.RemoveListDataFromFile(strFileName, lstTable) #Removes data from a file of a list of dictionary rows
        continue  # to show the menu
    ...
    # Step 3.4 - Save tasks to the ToDoFile.txt file
        ...
        if("y" == str(input("Save this data to file? (y/n) - ")).strip().lower()):  # Double-check with user
            ...
            # ToDo/done: Place processing code in a New function
            FileProcessor.WriteListDataToFile(strFileName, lstTable) # Write each row of data to the file
    ...
    # Step 3.5 - Reload data from the ToDoFile.txt file (clears the current data from the list/table)
        ...
        if (strYesOrNo.lower() == 'y'):
            ...
            FileProcessor.ReadFileDataToFile(strFileName, lstTable)  # Replace the current list data with file data
            IO.ShowCurrentItemsInList(lstTable)  # Show current data in the list/table
        else:
